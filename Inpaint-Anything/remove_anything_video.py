...
if __name__ == "__main__":
    ...
    if Path(video_raw_p).exists():
        # Replace the mimread call with frame-by-frame reading
        # Get video metadata
        video_reader = imageio.get_reader(video_raw_p)
        fps = video_reader.get_meta_data()["fps"]
        
        # Frame-by-frame processing
        frame_ps = []
        all_frame = []  # We'll only store the first frame here for visualization later
        print(f"Extracting frames from {video_raw_p}...")
        
        # Save first frame for visualization and reference
        first_frame = video_reader.get_data(0)
        all_frame.append(first_frame)
        
        # Process all frames
        for i, frame in enumerate(video_reader):
            frame_p = str(mkstemp(suffix=f"{i:0>6}.png"))
            frame_ps.append(frame_p)
            iio.imwrite(frame_ps[i], frame)
            
            # Only keep first frame in memory
            if i == 0:
                continue
            ...
            # Break if we've hit the frame limit
            if i >= num_frames - 1:
                break
        ...
        # Only load the first frame into memory initially
        all_frame = [iio.imread(frame_ps[0])]
        fps = 25
        # save tmp video
        # Don't try to write the whole video, just note the path
        print(f"Using frames from {frame_raw_glob}")
    ...
    tmp = []
    for i in range(len(all_mask)):
        # Load each frame as needed instead of using the all_frame list
        if i == 0 and len(all_frame) > 0:
            frame = all_frame[0]
        else:
